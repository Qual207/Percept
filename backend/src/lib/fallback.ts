...
/**
 * Keyword-based fallback intent parser.
 * Used when the LLM is unavailable (no API key, network error, demo safety).
 *
 * Maps free-form transcripts to one of three "states" via keyword matching,
 * then emits a deterministic plan for that state. Composable: if a transcript
 * matches multiple states, actions are merged and deduplicated.
 */

type State = "overloaded" | "distracted" | "sensory_overload" | "bigger_text" | "simpler";
...
/**
 * Merge plans by concatenating actions and deduplicating by (layer, type, selector).
 * Later actions of the same key override earlier ones (last write wins).
 */
...
export function fallbackPlan(transcript: string): Plan {
  ...
  if (states.length === 0) {
    return {
      reason_short: "No clear adaptation detected — light cleanup",
      intensity: 0.3,
      actions: [
        { layer: "structural", type: "centerMain", selector: "[data-an-role='main']" },
